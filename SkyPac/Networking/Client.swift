...
open class Client {        
    ...
    public func load<T, APIError>(resource: Resource<T, APIError>,
                                     completion: @escaping (Result<T, APIError>) ->()) -> URLSessionDataTask? {
...
extension URLRequest {
    init<T, E>(baseUrl: String, resource: Resource<T, E>) {
        var url = URL(baseUrl: baseUrl, resource: resource)
        url = url.addURLParameters(items: resource.queryItems!)
        self.init(url: url)
